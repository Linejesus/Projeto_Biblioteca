...
public class LivroService {
    ...
    public List<LivroResumoDTO> listarTodos(){
        ...
        public Livro alterarLivro(String id, LivroResumoDTO dto){
                ...
                livro.setGeneros(dto.getGeneros());
                livro.setEditoras(dto.getEditoras());
                livro.setEdicoes(dto.getEdicoes());
        ...
        // remove do Neo4j
        livroNodeRepository.deletarLivroNode(id);

        // remove do MongoDB
        livroRepository.delete(livro);
        }

        public List<LivroHomeDTO> recomendarLivros(Long idUsuario) {

                List<LivroNode> recomendados =
                        pessoaRepository.recomendarLivros(idUsuario);

                return recomendados.stream()
                        .map(node -> {

                                Livro livro = livroRepository
                                        .findById(node.getIdLivro())
                                        .orElse(null);
